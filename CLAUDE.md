...
plus `workflow_dispatch` and pushes to `main`), then copies `output/briefing.html`
(as `index.html`), `manifest.json`, and `icons/` into the Pages artifact and
deploys it to https://for-the-crown.github.io/morning-summary/. The repo is
public because GitHub Pages requires that on the free plan; there's nothing
sensitive in the code or its output.

## Home screen icons

`manifest.json` and the `<link rel="apple-touch-icon">`/`apple-mobile-web-app-*`
tags in `render()` let the page be added to a phone's home screen as a
standalone app (no browser chrome). The icon PNGs in `icons/` are generated by
`scripts/generate_icons.py`, a pure-stdlib PNG encoder (zlib + struct, no
image library) that draws a gold "C" ring on the masthead's ink background.
Re-run it after changing the mark; it isn't part of the daily workflow since
